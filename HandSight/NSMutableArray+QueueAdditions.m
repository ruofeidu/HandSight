...
-(id) popTail {
    id queueObject = nil;
    int lastID = (int)[self count] - 1;
    // Do we have any items?
    if ([self lastObject]) {
        // Pick out the first one
#if !__has_feature(objc_arc)
        queueObject = [[[self objectAtIndex: lastID] retain] autorelease];
#else
        queueObject = [self objectAtIndex: lastID];
#endif
        // Remove it from the queue
        [self removeObjectAtIndex: lastID];
    }
...
-(void) markHeadSpoken {
    NSString* s = [self peekHead];
    [self setObject:[NSString stringWithFormat:@"%@$$$", s] atIndexedSubscript:0];
}
